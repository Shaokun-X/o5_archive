...
class TileFitter:
	def __init__(self, tiles_data, mosaic_num):
		self.tiles_data = tiles_data
		self.mosaic_num = mosaic_num
		self.chosen_tile_index = set()
	...
	def get_best_fit_tile(self, img_data, repeat=True):
		...
		for i, tile_data in enumerate(self.tiles_data):
			if not repeat and i in self.chosen_tile_index:
				continue
			...
			tile_index += 1
			if not repeat:
				self.chosen_tile_index.add(best_fit_tile_index)
				# if a certain number of tiles has been used, then flush the chosen index and start again,
				# the factor decides when to flush in an adaptive way,
				# it ensures that there are always tiles who are "good enough"
				factor = self.mosaic_num / (0.15 * len(self.tiles_data)) + 1
				if ceil(len(self.tiles_data) / factor) == len(self.chosen_tile_index):
					self.chosen_tile_index = set()
		return best_fit_tile_index
	
	def get_random_fit_tile(self, img_data, candidate_num=5):
		if candidate_num - 1 > len(self.tiles_data):
			raise IndexError("Candidate number more than size of tiles")
		min_diff = sys.maxsize
		best_match = sorted(
			list(range(len(self.tiles_data))),
			key=lambda i : self.__get_tile_diff(img_data, self.tiles_data[i], min_diff)	
		)[:candidate_num]
		return random.choice(best_match)

def fit_tiles(work_queue, result_queue, tiles_data, mosaic_num, repeat):
	# this function gets run by the worker processes, one on each CPU core
	tile_fitter = TileFitter(tiles_data, mosaic_num)
	...
	while True:
		try:
			...
			if img_data == EOQ_VALUE:
				break
			tile_index = tile_fitter.get_best_fit_tile(img_data, repeat)
			# tile_index = tile_fitter.get_random_fit_tile(img_data)
...
def compose(original_img, tiles, repeat):
	...
	# how many mosaic in the final image
	mosaic_num = original_img_small.size[0] * original_img_small.size[1]
	...
	try:
		...
		for n in range(WORKER_COUNT):
			Process(target=fit_tiles, args=(work_queue, result_queue, all_tile_data_small, mosaic_num, repeat)).start()

		progress = ProgressCounter(mosaic.x_tile_count * mosaic.y_tile_count)

		# shuffle the iteration order of pixels, so that if there is not enough tiles, 
		# every pixel has the same chance to choose the most appropriate tile
		shuffled_row_indice = list(range(mosaic.x_tile_count))
		shuffled_column_indice = list(range(mosaic.y_tile_count))
		random.shuffle(shuffled_row_indice)
		random.shuffle(shuffled_column_indice)

		for x in shuffled_row_indice:
			for y in shuffled_column_indice:
...
def mosaic(img_path, tiles_path, repeat=False):
	image_data = TargetImage(img_path).get_data()
	tiles_data = TileProcessor(tiles_path).get_tiles()
	compose(image_data, tiles_data, repeat)

if __name__ == '__main__':
    mosaic('image/tankman.jpg', 'image/tiles', True)
	# if len(sys.argv) < 3:
